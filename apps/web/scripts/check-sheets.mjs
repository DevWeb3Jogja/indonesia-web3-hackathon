...
// --- 1. env var lengkap ---
const missing = ["GOOGLE_SHEET_ID", "GOOGLE_SERVICE_ACCOUNT_EMAIL", "GOOGLE_PRIVATE_KEY"].filter(
  (k) => !process.env[k]
);
...
if (!email.endsWith(".iam.gserviceaccount.com")) {
  console.log(`${NO} Email service account terlihat tidak biasa: ${email}`);
...
try {
  ...
  if (probe) {
    try {
      ...
      console.log(dim(`   catatan: tab sementara "${probeTitle}" gagal dihapus, hapus manual.`));
    }
  }
}
