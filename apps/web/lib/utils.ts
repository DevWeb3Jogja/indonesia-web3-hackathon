...
export function validateSubmission(b: Partial<SubmissionInput>): string | null {
  ...
  if (!Array.isArray(b.tracks) || b.tracks.length === 0) return "Pilih minimal satu track";
  const validTracks = TRACKS.map((t) => t.id) as string[];
  if (b.tracks.some((t) => !validTracks.includes(t))) return "Track tidak dikenal";
  if (!b.contractAddress || !isValidAddress(b.contractAddress))
    return "Contract address tidak valid (format 0x...)";
  if (!b.network || !NETWORKS.some((n) => n.id === b.network)) return "Network tidak valid";
  ...
  if (!b.githubUrl || !isValidUrl(b.githubUrl)) return "Link GitHub repo tidak valid";
  if (!b.demoVideoUrl || !isValidUrl(b.demoVideoUrl)) return "Link video demo tidak valid";
  ...
  if (b.teamMembers.some((m) => !m.name?.trim())) return "Nama anggota tim wajib diisi";
  if (Array.isArray(b.extraLinks) && b.extraLinks.some((l) => l.url && !isValidUrl(l.url)))
